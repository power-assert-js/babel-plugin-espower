...
class TinyMatcher {
    constructor(str) {
        const parsed = signature.parse(str);
        if (parsed === null) {
            throw new Error('Argument should be in the form of CallExpression');
        }
        this.callee = parsed.callee;
        this.args = parsed.args;
        this.numMaxArgs = this.args.length;
        this.numMinArgs = this.args.filter((a) => a.optional === false).length;
    }
    test(currentNode) {
