...
var fs = require('fs');
var argumentRecorderHelperCode = '(' +
  fs.readFileSync(require.resolve('./argument-recorder.js'), 'utf8')
    .split('\n')
    .slice(2)
    .join('\n')
  + ')()';
var assertionRecorderHelperCode = '(' +
  fs.readFileSync(require.resolve('./assertion-recorder.js'), 'utf8')
    .split('\n')
    .slice(2)
    .join('\n')
  + ')()';
...
function BabelAssertionVisitor (babel, matcher, options) {
    ...
    this.argumentRecorders = [];
    this.messageUpdated = false;
    this.locationDetector = new EspowerLocationDetector(this.options);
    var babelTemplate = babel.template;
    this.argumentRecorderTemplate = babelTemplate(argumentRecorderHelperCode);
    this.assertionRecorderTemplate = babelTemplate(assertionRecorderHelperCode);
...
BabelAssertionVisitor.prototype.leave = function (nodePath) {
    var currentNode = nodePath.node;
    var visitorKeys = this.options.visitorKeys;
    var modifiedSome = this.argumentModifiedHistory.some(function(b) { return b; });
    try {
        return modifiedSome ? this.captureAssertion(nodePath) : currentNode;
    } finally {
        this.argumentRecorders = [];
        this.argumentModifiedHistory = [];
        this.messageUpdated = false;
...
BabelAssertionVisitor.prototype.leaveArgument = function (resultTree, nodePath) {
    try {
        var currentNode = nodePath.node;
        var parentNode = nodePath.parent;
        var argMatchResult = this.matcher.matchArgument(currentNode, parentNode);
        ...
        var resultNode = this.argumentModified ? this.captureArgument(resultTree) : resultTree;
        if (argMatchResult.name === 'message' && argMatchResult.kind === 'optional') {
            this.messageUpdated = true;
            return this.createNewAssertionRecorder(nodePath, resultNode);
        } else {
            return resultNode;
        }
    } finally {
        ...
        this.argumentModified = false;
        this.argumentRecorders.push(this.argumentRecorder);
        this.argumentRecorder = null;
    }
};
...
BabelAssertionVisitor.prototype.parseCanonicalCode = function (file, code) {
    ...
    function doParse(wrapper) {
        ...
        var output = babelParser.parse(content, assign({}, file.opts.parserOpts, {tokens: true}));
        if (wrapper) {
...
BabelAssertionVisitor.prototype.captureAssertion = function (nodePath) {
    var currentNode = nodePath.node;
    if (this.messageUpdated) {
        // AssertionRecorder is already merged with existing message argument
        return currentNode;
    }
    var sigs = this.matcher.argumentSignatures();
    var numParams = sigs.length;
    var lastArg = sigs[numParams - 1];
    var hasOptionalMessageArgument = (lastArg.name === 'message' && lastArg.kind === 'optional');
    var actualNumArgs = this.argumentModifiedHistory.length;
    var onlyLastArgumentIsOmitted = (numParams - actualNumArgs === 1);
    if (onlyLastArgumentIsOmitted && hasOptionalMessageArgument) {
        // appending AssertionRecorder in place of omitted message argument
        currentNode.arguments.push(this.createNewAssertionRecorder(nodePath));
    } else {
        // NEEDS PLAN B
    }
    return currentNode;
...
BabelAssertionVisitor.prototype.createNewAssertionRecorder = function (nodePath, originalMessageNode) {
    var _this = this;
    var t = this.babel.types;
    var props = {
    ...
    if (originalMessageNode) {
        propsNode.properties.push(t.objectProperty(t.identifier('message'), originalMessageNode));
    }
    var assertionRecorderNameNode = this.getOrCreateNode(nodePath, 'PowerAssertAssertionRecorder', function () {
        return t.toExpression(_this.assertionRecorderTemplate());
    });
    var newNode = t.newExpression(assertionRecorderNameNode, [
        t.arrayExpression(this.argumentRecorders), // CAUTION: cannot create before argumentRecorders
        propsNode
    ]);
    define(newNode, { _generatedByEspower: true });

    //// PoC recorder with assignment
    // var assertionRecorderIdent = nodePath.scope.generateUidIdentifier('ar');
    // define(assertionRecorderIdent, { _generatedByEspower: true });
    // nodePath.scope.push({ id: assertionRecorderIdent, init: newNode });
    // currentNode.arguments.push(assertionRecorderIdent);
    return newNode;
};
...
BabelAssertionVisitor.prototype.createNewArgumentRecorder = function (nodePath) {
    ...
    var types = this.babel.types;
    var currentNode = nodePath.node;
    var argRecNameNode = this.getOrCreateNode(nodePath, 'PowerAssertArgumentRecorder', function () {
        return types.toExpression(_this.argumentRecorderTemplate());
    });
    var recorderIdent = nodePath.scope.generateUidIdentifier('ag');
    define(recorderIdent, { _generatedByEspower: true });
    var gen = new babelgen.CodeGenerator(currentNode, { concise: true, comments: false });
    var output = gen.generate();
    var argumentCanonicalCode = output.code;
    var relativeEsPath = currentNode._espowerEspath;
    var init = types.newExpression(argRecNameNode, [
