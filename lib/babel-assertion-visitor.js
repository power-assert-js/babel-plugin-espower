...
BabelAssertionVisitor.prototype.generateCanonicalCode = function (nodePath, node) {
    ...
    this.visitorKeys = this.getVisitorKeysNode(nodePath);
};
...
BabelAssertionVisitor.prototype.getVisitorKeysNode = function (nodePath) {
    var file = nodePath.hub.file;
    var visitorKeysIdent = file.get('powerAssertVisitorKeys');
    if (!visitorKeysIdent) {
        visitorKeysIdent = this.createVisitorKeysNode(nodePath);
    }
    return visitorKeysIdent;
};

BabelAssertionVisitor.prototype.createVisitorKeysNode = function (nodePath) {
    var types = this.babel.types;
    var file = nodePath.hub.file;
    var programScope = nodePath.scope.getProgramParent();
    var visitorKeysIdent = programScope.generateUidIdentifier('powerAssertVisitorKeys');
    define(visitorKeysIdent, { _generatedByEspower: true });
    file.set('powerAssertVisitorKeys', visitorKeysIdent);
    var visitorKeysJsonStringLiteral = types.stringLiteral(JSON.stringify(this.options.visitorKeys));
    define(visitorKeysJsonStringLiteral, { _generatedByEspower: true });
    visitorKeysJsonStringLiteral._compact = true;
    programScope.push({ id: visitorKeysIdent, init: visitorKeysJsonStringLiteral });
    return visitorKeysIdent;
};
