...
class BabelAssertionVisitor {
  ...
  enter (nodePath) {
    ...
    this.configIdent = this.generatePowerAssertConfig(nodePath, source);
    // should be after configIdent
    // should be after enclosingFunc detection
    this.metadataIdent = this.generateMetadata(nodePath, line);
  ...
  generatePowerAssertConfig (nodePath, filepath) {
    const types = this.babel.types;
    ...
    const generateNode = () => {
      ...
      const funcNode = types.arrowFunctionExpression([
        ...
        types.variableDeclaration('const', [
          ...
          types.variableDeclarator(filepathIdent, types.stringLiteral(filepath)),
        ...
        types.returnStatement(types.objectExpression([
          ...
          types.spreadElement(extraIdent),
          types.spreadElement(types.memberExpression(patternsIdent, patternIndexIdent, true))
        ]))
      ]));
      return funcNode;
    };
  ...
  generateMetadata (nodePath, lineNumber) {
