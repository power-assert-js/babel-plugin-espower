...
class BabelAssertionVisitor {
  ...
  generatePowerAssertConfig (nodePath) {
    ...
    const { source } = locationDetector.locationFor(nodePath.node);
    const types = this.babel.types;
    const options = this.options;
    const visitorKeys = options.visitorKeys;
    const keyName = '_pwcfg';
    const generateNode = () => {
      ...
      const idxIdent = types.identifier('idx');
      ...
      const funcNode = types.arrowFunctionExpression([
        ...
        types.variableDeclaration('const', [
          ...
          types.variableDeclarator(filepathIdent, types.stringLiteral(source)),
        ...
        types.returnStatement(types.objectExpression([
          ...
          types.spreadElement(types.memberExpression(patternsIdent, idxIdent, true))
        ]))
      ]));
      return funcNode;
    };
  ...
  generateMetadata (nodePath) {
    ...
    const locationDetector = new EspowerLocationDetector(this.options);
    const { line } = locationDetector.locationFor(nodePath.node);
    const props = {
    ...
    const callConfigFuncNode = t.callExpression(this.configIdent, [
      t.numericLiteral(this.patternMatcher.index)
    ]);
    propsNode.properties.push(t.spreadElement(callConfigFuncNode));
