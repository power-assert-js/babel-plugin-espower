...
BabelAssertionVisitor.prototype.enterArgument = function (nodePath) {
    ...
    var argMatchResult = this.matcher.matcher.matchArgument(currentNode, parentNode);
    if (!argMatchResult) {
        return;
    }
...
BabelAssertionVisitor.prototype.leaveArgument = function (nodePath) {
    var currentNode = nodePath.node;
    var shouldCapture = this.argumentModified || this.toBeCaptured(nodePath);
    try {
        var resultNode = shouldCapture ? this.captureArgument(nodePath) : currentNode;
...
BabelAssertionVisitor.prototype.appendMessage = function (nodePath) {
    ...
    var sigs = this.matcher.matcher.argumentSignatures();
...
BabelAssertionVisitor.prototype.generateMetadata = function (nodePath) {
    ...
    if (typeof this.matcher.index === 'number') {
        var configAccess = t.memberExpression(this.configIdent, t.valueToNode(this.matcher.index), true);
        var configNode = t.objectProperty(t.identifier('config'), configAccess);
...
BabelAssertionVisitor.prototype.createNewArgumentRecorder = function (nodePath) {
    ...
    }, this.currentArgumentMatchResult);
