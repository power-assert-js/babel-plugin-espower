...
class BabelAssertionVisitor {
  ...
  generateAstAndTokensAndVisitorKeys (nodePath, canonicalCode) {
    ...
    this.visitorKeys = this.getOrCreateNode({ nodePath, keyName, generateNode, visitorKeys });
  }
  ...
  appendMessage (nodePath) {
    ...
    if (onlyLastArgumentIsOmitted) {
      ...
      currentNode.arguments.push(this.createNewAssertionMessage(params));
      ...
      if (this.toBeSkipped(lastActualArg)) {
        ...
        currentNode.arguments[lastIndex] = this.createNewAssertionMessage(Object.assign(params, { originalMessageNode, matchIndex }));
      }
    }
    return currentNode;
  }
  ...
  generatePowerAssertConfig (nodePath) {
    ...
    return this.getOrCreateNode({ nodePath, keyName, generateNode, visitorKeys });
  }
  ...
  createNewAssertionMessage ({ nodePath, types, visitorKeys, metadataIdent, cloneAst, originalMessageNode = null, matchIndex = -1 }) {
    const keyName = 'AssertionMessage';
    const generateNode = () => {
      return types.toExpression(assertionMessageTemplate(cloneAst));
    };
    const assertionMessageNameNode = this.getOrCreateNode({ nodePath, keyName, generateNode, visitorKeys });
    const newArgs = [
      metadataIdent,
  ...
  getOrCreateNode ({ nodePath, keyName, generateNode, visitorKeys }) {
    const file = nodePath.hub.file;
    // helperNameNode = file.addImport('power-assert-runtime/recorder', 'default', 'recorder');
    return file.get(keyName) || this.createNode({ nodePath, keyName, generateNode, visitorKeys });
  }

  createNode ({ nodePath, keyName, generateNode, visitorKeys }) {
    const file = nodePath.hub.file;
