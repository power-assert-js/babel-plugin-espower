...
var argumentRecorderHelperCode = '(' +
  fs.readFileSync(require.resolve('./argument-recorder.js'), 'utf8')
    ...
    .join('\n')
  + ')()';
var assertionMessageHelperCode = '(' +
...
function BabelAssertionVisitor (babel, matcher, options) {
    ...
    this.currentArgumentEsPath = null;
    ...
    this.assertionMessageTemplate = babelTemplate(assertionMessageHelperCode);
...
BabelAssertionVisitor.prototype.enterArgument = function (nodePath) {
    ...
    this.currentArgumentEsPath = currentNode._espowerEspath;
...
BabelAssertionVisitor.prototype.leaveArgument = function (resultTree, nodePath) {
    try {
        ...
        if (this.currentArgumentMatchResult.name === 'message' && this.currentArgumentMatchResult.kind === 'optional') {
            ...
            return this.createNewAssertionMessage(nodePath, resultNode);
        } else {
            return resultNode;
        }
    } finally {
...
BabelAssertionVisitor.prototype.captureNode = function (nodePath) {
    ...
    var isBlockArgument = (relativeEsPath === this.currentArgumentEsPath) &&
        (this.currentArgumentMatchResult.index === 0) &&
        (this.currentArgumentMatchResult.name === 'block') &&
        (this.currentArgumentMatchResult.kind === 'mandatory');
    var newNode = t.callExpression(
        t.memberExpression(this.argumentRecorder, t.identifier(isBlockArgument ? '_blk' : '_tap')),
        [
            currentNode,
...
BabelAssertionVisitor.prototype.appendMessage = function (nodePath) {
    ...
    var numDefinedParams = sigs.length;
    var lastParam = sigs[numDefinedParams - 1];
    var hasOptionalMessageArgument = (lastParam.name === 'message' && lastParam.kind === 'optional');
    if (!hasOptionalMessageArgument) {
        return currentNode;
    }
    var numActualArgs = currentNode.arguments.length;
    var onlyLastArgumentIsOmitted = (numDefinedParams - numActualArgs === 1);
    if (onlyLastArgumentIsOmitted) {
        // appending AssertionMessage in place of omitted message argument
        currentNode.arguments.push(this.createNewAssertionMessage(nodePath));
    } else if (numDefinedParams === numActualArgs) {
        var lastActualArg = currentNode.arguments[numActualArgs - 1];
        if (this.toBeSkipped(lastActualArg)) {
            // last arg may be a string literal
            currentNode.arguments[numActualArgs - 1] = this.createNewAssertionMessage(nodePath, lastActualArg);
        }
    }
    return currentNode;
};
...
BabelAssertionVisitor.prototype.generateMetadata = function (nodePath) {
    ...
    var metadataIdent = nodePath.scope.generateUidIdentifier('am');
    define(metadataIdent, { _generatedByEspower: true });
    nodePath.scope.push({ id: metadataIdent, init: propsNode });
    return metadataIdent;
};

BabelAssertionVisitor.prototype.createNewAssertionMessage = function (nodePath, originalMessageNode) {
    var _this = this;
    var t = this.babel.types;
    var assertionMessageNameNode = this.getOrCreateNode(nodePath, 'AssertionMessage', function () {
        return t.toExpression(_this.assertionMessageTemplate());
    });
    var newArgs = [
    ...
    var newNode = t.newExpression(assertionMessageNameNode, newArgs);
    define(newNode, { _generatedByEspower: true });
    return newNode;
};
...
BabelAssertionVisitor.prototype.createNewArgumentRecorder = function (nodePath) {
    ...
    var types = this.babel.types;
    var currentNode = nodePath.node;
    var argRecNameNode = this.getOrCreateNode(nodePath, 'ArgumentRecorder', function () {
