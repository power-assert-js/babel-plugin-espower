...
function BabelEspowerVisitor (babel, opts) {
    this.babel = babel;
    this.matchers = opts.patterns.map(function (pattern, idx) {
        ...
        var matcher = {
            matcher: new CallMatcher(expression, opts),
            index: idx
        };
        if (typeof pattern === 'object') {
...
BabelEspowerVisitor.prototype.enter = function (nodePath) {
    ...
    if (assertionVisitor) {
        ...
        var matcher = find(this.matchers, function (m) { return m.matcher.test(currentNode); });
        if (matcher) {
