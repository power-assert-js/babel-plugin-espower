...
function hasOptionalMessageArgument (matcher) {
    var signatures = matcher.argumentSignatures();
    var lastArg = signatures[signatures.length - 1];
    return (lastArg.name === 'message' && lastArg.kind === 'optional');
}
...
BabelEspowerVisitor.prototype.exit = function (nodePath) {
    ...
    if (assertionVisitor.isLeavingAssertion(nodePath)) {
        // leaving assertion
        if (hasOptionalMessageArgument(assertionVisitor.matcher)){
